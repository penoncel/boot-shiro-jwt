...
public class CodeRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        // 从redis中获取登录验证码
        String redisCode =  redisService.getString(LogingUserKey.smsCode,token.getUsername());
        if (redisCode == null) {
        ...
        if(redisService.exists(LogingUserKey.loginUserIinfo,token.getUsername())){
            loginUser = redisService.get(LogingUserKey.loginUserIinfo,token.getUsername(), LoginUser.class);
        }else{
